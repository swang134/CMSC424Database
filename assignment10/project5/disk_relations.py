...
class RelationBlock:
	...
	def initializeFromJSON(self, js):
		self.tuples = [RelationTuple(self.schema, t) for t in byteify(json.loads(js))]
	def __str__(self):
		return self.toJSON()
	def getTuple(self, primary_id):
		for t in self.tuples:
			if t.getAttribute('ID') == primary_id: #.decode()
				return t
		assert False
...
class BufferPool:
	...
	LRUOrder = list(range(0, Globals.BufferPoolSize))
