...
class LockTable:
	...
	def cycleRemoval(graph):
		visited = []

		def helper(k):
			visited.append(k)
			for x in graph[k]:
				if x in graph and (x in visited or helper(x)):
					return True
			return False
...
class LogManager:
	...
	def restartRecovery():
		...
		relat = ""
		for r in allrecords:
			if r.info[1] == LogRecord.START:
				active.append(r.info[0])
			elif r.info[1] == LogRecord.COMMIT:
				active.remove(r.info[0])
				LogManager.redoChanges(r.info[0])
			elif r.info[1] == LogRecord.ABORT:
				active.remove(r.info[0])
			else:	
				relat = r.info[2]

		for a in active:
			LogManager.revertChanges(a)
			LogManager.createAbortLogRecord(a)

		relat = Relation.getRelationByName(relat)
		BufferPool.writeAllToDisk(relat)
	...
	@staticmethod
	def redoChanges(transactionid):
		with LogManager.logfile_lock:
			f = open(LogManager.fileName, 'r')
			allrecords = [LogManager.readLogRecord(line) for line in f.readlines()]
			f.close()
		redorecords = [lr for lr in allrecords if lr.info[0] == transactionid]
		for lr in redorecords:
