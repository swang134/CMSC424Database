...
class BTreeBlock(Block):
	...
	def findSiblingWithSameParent(self, parentBlock):
		...
		for index in range(0, len(parentBlock.keysAndPointers), 2):
			...
			if parentBlock.keysAndPointers[index].blockNumber == self.blockNumber:
				if index == (len(parentBlock.keysAndPointers) - 1):
	...
	# Refer to Figure 11.19 from the textbook starting from "else begin /* Redistribution: ..."
	# self is the successor, otherBlock is the predecessor, and kprime is the 
	# key between the two siblings in the parent block.
	...
	def mergeOrRedistributeWithSibling(self):
		...
		if block1.canMergeWith(block2):
			...
			# NOTE: block2 is the calling block
			newKey = block2.redistributeWithBlock(block1, key)

			# Replace key with newKey in the parent
			for index in range(0, len(parentBlock.keysAndPointers), 2):
				if parentBlock.keysAndPointers[index].blockNumber == block2.blockNumber:
