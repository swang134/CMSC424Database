# CMSC424 Spring 2021 Assignment 1: Computing Environment (ungraded)
...
We will be pulling from the CS department's GitLab.

`git clone https://gitlab.cs.umd.edu/keleher/424s21`

You can do `git pull` (from within the `424s21` directory) to fetch newly added material (when new assignments, or changes to existing assignments, are released). 
...
- You can not push changes to our gitlab account, but feel free to submit *pull requests* if you spot errors or come up with better explanations.
- Familiarize yourself with the basic git commands
    - At a minimum, you should know: `clone`, `add`, `commit`, `push`, `pull`, `status`.
    - But you should also be familiar with how to use **branches**.

#### Working with GitHub/GitLab
We will not be using GitHub at all, but you may want to get your own account to host your work, keep track of changes, etc.
...
- **NOTE:** During the installation vagrant will say that there is a more recent version of the VM. **Ignore.**. When you SSH into the VM it will tell you that a reboot is necessary. **Ignore.** In both cases, problems usually result.
